...
#' (scaling based on user-supplied reference min and max bounds). Values are
#' scaled to a 0 to 100 range.
#'
#' @param x A numeric vector to standardize.
#' @param selected_method A character string indicating the scaling method.
#'   Must be either \code{"observe"} (default) or \code{"benchmark"}.
#' @param min_bound A numeric value representing the minimum reference bound.
#'   Required if \code{selected_method} is \code{"benchmark"}.
#'   Default is \code{NULL}.
#' @param max_bound A numeric value representing the maximum reference bound.
#'   Required if \code{selected_method} is \code{"benchmark"}.
#'   Default is \code{NULL}.
#'
#' @return
#' A numeric vector of the same length as \code{x}, where the values have been
...
#'
#' @export
s_standardize <- function(x, selected_method = "observe",
                          min_bound = NULL, max_bound = NULL) {
  # Validate input method
  if (!selected_method %in% c("observe", "benchmark")) {
    stop("`selected_method` must be either 'observe' or 'benchmark'.")
  }

  # Defensive check for benchmark requires min_bound and max_bound
  if (selected_method == "benchmark") {
    if (is.null(min_bound) || is.null(max_bound)) {
      stop(
        paste(
  ...
  if (selected_method == "observe") {
    scaled_vec <- s_standardize_vector(
      v = x,
